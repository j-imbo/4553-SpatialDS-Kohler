...
def hueshift(color,deg): #shifts a hue by a certain degree
    color = color[1:] # "#000000" -> "000000"
    hexin = [int(color[i:i+2],16) for i in range(0,len(color),2)] # 010203 -> [01,02,03]
    ...
    hexout = [0,0,0]
    for i in range(3):
        hexout[i] = math.floor(abs(hexin[0]*rotmat[i][0] + hexin[1]*rotmat[i][1] + hexin[2]*rotmat[i][2]))
...
geo['features'] = sorted(geo['features'],key=lambda x:x['properties']['pop'])
