...
class quadDriver(object):
    ...
    def initPts(self,pts):
        while self.pid < pts:
            ...
            pt = qt.Point(x,y,id=self.pid,color=self.ptcolor,radius=self.ptrad,hili=False)
            self.tree.insert(pt)
            self.pts.append(pt)
            self.pid += 1
...
class GDrive(object):
    ...
    def captureEvents(self):
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                if event.button == 1:
                    if 511 in self.events["keysPressed"]: # figure out pg.K_LCTRL
    ...
    def updateLogic(self,events):
        if "data" in events:
            if "startPos" in self.events["data"] and "endPos" in self.events["data"]:
                ...
                w = abs(x1-x2)
                h = abs(y1-y2)
                self.quad.recs.append(pg.Rect(l,t,w,h))
                del self.events["data"]["startPos"]
                del self.events["data"]["endPos"]
            if 32 in self.events["keysPressed"]: # figure out pg.SPACE
        ...
        try:
            ...
            if self.scanrec.bottom > HEIGHT:
                ...
                self.scanrec.left = 0
                self.scanrec.top += 64
            self.qscan = qt.Rect(self.scanrec.centerx,self.scanrec.centery,64,64)
            for pt in pts:
        ...
        for rec in recs:
            qrec = qt.Rect((rec.left+rec.right)/2,(rec.top+rec.bottom)/2,rec.width,rec.height)
            for pt in pts:
